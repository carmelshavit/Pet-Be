...
const findEmailUser = (email) => {
	return {
		sql: `SELECT * FROM defaultdb.users WHERE email = ?`,
		values: [email],
	};
};
...
const getUserByIdQuery = (userId) => {
	return {
		sql: `SELECT * FROM defaultdb.users WHERE id = ?`,
		values: [userId],
	};
};
...
const getUsersQuery = (filters) => {
	try {
		let query = 'SELECT * from defaultdb.users';
		const conditions = [];
...
const addUserQuery = () => {
	return {
		sql: `INSERT INTO defaultdb.users SET ?`,
		values: [],
	};
};
...
const addPetQuery = () => {
	return {
		sql: `INSERT INTO defaultdb.pets SET ?`,
		values: [],
	};
};

const getPetsByAndQuery = (filters) => {
	let query = 'SELECT * from defaultdb.pets';
...
const getPasswordUserQuery = (userId) => {
	return {
		sql: `SELECT password FROM defaultdb.users WHERE id = ?`,
		values: [userId],
	};
};
...
const editUserQuery = (editedUser) => {
	...
	if (setClauses.length !== 0 && editedUser.id) {
		...
		return {
			sql: `UPDATE defaultdb.users SET ${setClause} WHERE id = ?`,
...
const editPetQuery = (petId, editedPet) => {
	...
	if (setClauses.length !== 0 && petId) {
		const setClause = setClauses.join(', ');
		return {
			sql: `UPDATE defaultdb.pets SET ${setClause} WHERE id = ?`,
...
const addLikeQuery = (userId, petId) => {
	return {
		sql: `INSERT INTO defaultdb.pet_status (userId, petId) VALUES(?, ?)`,
		values: [userId, petId],
	};
};
...
const getLikedPetsQuery = (likedPetIds) => {
	return {
		sql: `SELECT * FROM defaultdb.pets WHERE id IN (?)`,
		values: [likedPetIds],
	};
};
...
const getLikesQuery = (userId, petId) => {
	return {
		sql: `SELECT * FROM defaultdb.pet_status WHERE userId = ? AND petId = ?`,
		values: [userId, petId],
	};
};
...
const returnPetQuery = (petId, userId) => {
	return {
		sql: `UPDATE defaultdb.pets SET adoptedBy=NULL WHERE id = ? AND adoptedBy = ?`,
		values: [petId, userId],
	};
};
...
const adoptPetQuery = (petId, userId) => {
	return {
		sql: 'UPDATE defaultdb.pets SET adoptedBy = ? WHERE id = ?',
		values: [userId, petId],
	};
};
...
const removeLikeQuery = (userId, petId) => {
	return {
		sql: 'DELETE FROM defaultdb.pet_status WHERE userId = ? AND petId = ?',
		values: [userId, petId],
	};
};
